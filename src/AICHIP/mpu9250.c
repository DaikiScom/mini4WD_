...
void updateMPU9250Data (void)
{
	...
	for(i=0;i<3;i++)
	{
		acc_LPF_vec[i]     = gain_LPF * acc_LPF_vec[i] + (1.0-gain_LPF) * acc_vec[i];
		omega_LPF_vec[i]    = gain_LPF * omega_LPF_vec[i] + (1.0-gain_LPF) * omega_vec[i];
		acc_str_LPF_vec[i]     = gain_str_LPF * acc_LPF_vec[i] + (1.0-gain_str_LPF) * acc_vec[i];
		omega_str_LPF_vec[i]   = gain_str_LPF * omega_str_LPF_vec[i] + (1.0-gain_str_LPF) * omega_vec[i];
	}
